...
export class LeaderService {
    ...
    getFeaturedLeader(): Observable<Leader> {
        return this .http .get< Leader[] >( `${ BASE_URL }leadership?featured=true` )        // Returns an observable that contains an Array with Objects of Type Dish
                    .pipe(
                        map( leaders => leaders[ 0 ] )
                    )
                    .pipe( catchError( this .processHttpMessageService .handleError ) );              // Handle the error and extract the message                                     // Simulate server latency with 2 second delay.
    }
}
